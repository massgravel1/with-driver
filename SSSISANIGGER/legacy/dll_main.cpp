...
DWORD WINAPI SSSISANIGGER_main(HMODULE hModule)
{
    g_module = hModule;
    ...
    // NOTE(SSSISANIGGER): one second is enough... right?
    Sleep(1000);

    if (!init)
    {
        // NOTE(SSSISANIGGER): Compatibility Mode
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
    if (ul_reason_for_call == DLL_PROCESS_ATTACH)
    {
        ...
        CloseHandle(CreateThread(0, 0, (LPTHREAD_START_ROUTINE)SSSISANIGGER_main, hModule, 0, 0));
    }
    return TRUE;
}
